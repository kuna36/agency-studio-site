    ...
    <div className="site-shell noise">
      ...
      <main>
        ...
        <section id="work" className="section border-b border-white/[.06] bg-[#090f1b]">
          <div className="container-wide"><Reveal><SectionLabel index="06">Work in progress, built with intent</SectionLabel><div className="flex flex-col justify-between gap-7 md:flex-row md:items-end"><div><h2 className="section-title text-white">Our <span className="text-cyan-200">work.</span></h2><p className="body-copy mt-5 max-w-[490px]">Selected concepts, prototypes and demos that show how we think about useful digital products. No inflated claims — just the work and the problem behind it.</p></div><span className="mono text-[10px] text-slate-500">PROJECTS / CONCEPTS / DEMOS</span></div></Reveal><div className="mt-14 grid gap-4 md:grid-cols-2">{site.projects.map((project) => <Reveal key={project.title}><article className="glass-card glass-card-hover overflow-hidden"><div className={`project-visual ${project.accent}`}><div className="visual-grid" /><span className="visual-kicker">{project.category} / {project.label}</span><div className="visual-beam" /><div className="absolute bottom-7 left-8 z-[2] text-3xl font-extrabold tracking-[-.08em] text-white/90">{project.icon === "shield" ? <ShieldCheck size={38} strokeWidth={1} /> : project.icon === "waves" ? <Workflow size={38} strokeWidth={1} /> : project.icon === "chart" ? <BarChart3 size={38} strokeWidth={1} /> : <Sparkles size={38} strokeWidth={1} />}</div></div><div className="p-6"><div className="flex items-center justify-between gap-4"><h3 className="text-xl font-bold tracking-[-.04em] text-white">{project.title}</h3><span className="mono shrink-0 text-[9px] text-cyan-200/70">{project.label}</span></div><p className="mt-3 text-sm leading-7 text-slate-400">{project.description}</p><div className="mt-6 grid gap-3 border-t border-white/[.08] pt-5 text-xs leading-5"><div><span className="mono mr-2 text-[9px] text-slate-600">CHALLENGE</span><span className="text-slate-400">{project.problem}</span></div><div><span className="mono mr-2 text-[9px] text-slate-600">BUILT</span><span className="text-slate-400">{project.built}</span></div><div><span className="mono mr-2 text-[9px] text-slate-600">RESULT</span><span className="text-slate-400">{project.result}</span></div></div><button onClick={() => toast.info("Case study details can be connected here when the project is ready.")} className="mt-6 flex items-center gap-2 text-xs font-bold uppercase tracking-[.08em] text-cyan-200">View case study <ArrowUpRight size={14} /></button></div></article></Reveal>)}</div></div>
        </section>
